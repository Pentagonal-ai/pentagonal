...
export async function generateContract(prompt, chain, useRules = true, solanaType) {
...
export async function auditContract(code, chain, useRules = true) {
    const client = getClient();
...
// ─── Fix Vulnerability ───
export async function fixVulnerability(code, findingTitle, findingDescription) {
    const client = getClient();
    const response = await client.messages.create({
        model: MODEL,
        max_tokens: 8192,
