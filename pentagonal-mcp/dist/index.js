...
server.tool('pentagonal_score', 'Get the Pentagon Score (0-100, higher = safer) for a token or contract — a transparent risk score from honeypot, mintability, LP-lock, owner power, holder concentration, and audit findings. Returns the full factor-by-factor breakdown. EVM + Solana.', { address: z.string().describe('Contract/token address'), chain: SENTINEL_CHAIN.describe('Blockchain') }, async ({ address, chain }) => {
    try {
        ...
        return { content: [{ type: 'text', text: `❌ Score failed: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
});
server.tool('pentagonal_quantum_scan', 'Quantum exposure scan (crypto-agility analytics) — flags quantum-vulnerable cryptographic primitives in a verified EVM contract (secp256k1 via ecrecover/ECDSA/permit/EIP-1271, BN254 pairings) and recommends a migration path. Honest forward-risk analytics, not "quantum-proof" certification.', { address: z.string().describe('EVM contract address'), chain: SENTINEL_CHAIN.describe('Blockchain') }, async ({ address, chain }) => {
    try {
        ...
        return { content: [{ type: 'text', text: `⚛️ Quantum exposure: ${d.exposed ? 'EXPOSED' : 'clean'} (${d.severity})\nPrimitives: ${(d.primitives ?? []).join(', ') || 'none'}\n\n${findings}` }] };
    }
    catch (e) {
        return { content: [{ type: 'text', text: `❌ Quantum scan failed: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
});
server.tool('pentagonal_wallet_scan', 'Scan a wallet for risky token approvals (EVM). Flags unlimited allowances and malicious / flagged / unverified spender contracts so the user can revoke before getting drained.', { address: z.string().describe('Wallet address to scan'), chain: SENTINEL_CHAIN.describe('Blockchain') }, async ({ address, chain }) => {
    try {
        ...
        return { content: [{ type: 'text', text: `👛 Wallet approvals: ${d.totalApprovals ?? 0} total, ${(d.risky ?? []).length} risky\n\n${risky}` }] };
    }
    catch (e) {
        return { content: [{ type: 'text', text: `❌ Wallet scan failed: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
});
server.tool('pentagonal_forecast', 'Exploit forecasting — match a verified EVM contract against Pentagonal\'s accumulated exploit signatures to warn about likely attack surface BEFORE it is exploited. Returns matched signatures and an imminence rating.', { address: z.string().describe('EVM contract address'), chain: SENTINEL_CHAIN.describe('Blockchain') }, async ({ address, chain }) => {
    try {
        ...
        return { content: [{ type: 'text', text: `❌ Forecast failed: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
});
// ─── Start Server ───
async function main() {
